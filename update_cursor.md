...
## 2025-10-03 (晚) - 新增侧边栏收缩功能

### 🎯 更新目标
添加侧边栏收缩按钮，特别针对移动端优化，让用户可以全屏查看 MD 内容。

### 🚀 更新内容

#### 修改文件
- **static/js/script.js** - 添加侧边栏状态管理和切换功能
- **templates/index.html** - 添加切换按钮和遮罩层
- **static/css/style.css** - 添加收缩动画和移动端优化
...
## 2025-10-03 (晚) - 支持多目录扫描

### 🎯 更新目标
支持同时扫描多个目录的 MD 文件，解决配置文件重复键的问题。

### 🚀 更新内容

#### 修改文件
- **env.ini** - 改用逗号分隔多个目录
- **main.py** - 支持多目录扫描和文件路径映射

#### 核心功能

1. **多目录配置**
   - 使用逗号分隔多个扫描目录
   - 格式：`scanfolder=目录1,目录2,目录3`
   - 自动过滤空白目录
...
- ❌ 修复：`DuplicateOptionError` 配置文件重复键错误
- ✅ 支持：同时扫描多个目录
- ✅ 支持：跨目录的同名文件
- ✅ 改进：更灵活的配置方式
